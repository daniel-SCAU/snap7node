 ...
 *    goodBadRatio     %MD54      Real   byte 54, 4 bytes
 *    bagsPerMin       %MW62      Int    byte 62, 2 bytes
...
class PlcClient {
  ...
  readAll() {
    return new Promise((resolve, reject) => {
      ...
      const items = [
        ...
        { Area: AREA_MK, WordLen: WL_BYTE, DBNumber: 0, Start: 54,  Amount: 4 }, // goodBadRatio MD54
        { Area: AREA_MK, WordLen: WL_BYTE, DBNumber: 0, Start: 62,  Amount: 2 }, // bagsPerMin MW62
  ...
  _parseResults(results) {
    ...
    const goodBadRatioBuf  = check(results[7], 7);
    const bagsPerMinBuf    = check(results[8], 8);
